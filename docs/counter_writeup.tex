...
\hsprovenance{Figure 1 drawn from src/project.v at commit fd4d929, track/counter, 23 September 2026.}
\newpage
...
always @(posedge clk or negedge rst_n) begin
  ...
  end else if (do_load) begin  // do_load = load & ~oe
...
\item \textit{Load.} With \texttt{LOAD} high and \texttt{OE} low, the next rising edge copies the
  eight bits on the bus into the register. Load is checked before enable, so raising both loads the value and
  does not add one to it.
\item \textit{Count.} With \texttt{ENABLE} high and no load taking place, the next rising edge adds
...
\hsprovenance{Listing 1 copied from src/project.v, lines 65 to 74, at commit fd4d929; the do\_load comment is added.}
\newpage
...
No, and this is the price of sharing the bus. While \texttt{OE} is high the tile drives
\texttt{uio}, and the GF180 pad switches its input side off (its datasheet lists input and output
enabled together as disallowed), so \texttt{uio\_in} has no defined value. The design therefore
ignores \texttt{LOAD} while \texttt{OE} is high. To load, drop \texttt{OE}, put the value on the
bus, then raise \texttt{LOAD} for one clock edge.
\end{hscallout}
\newpage

\section{How it was verified}\label{sec:verify}
\hslead{Eight CocoTB tests, a check that they catch broken designs, and the Tiny Tapeout build.}

\begin{hsstatrow}[3]
\hsstat{8 / 8}{CocoTB tests passing in CI}
...
The broken-counter check was run in RTL simulation only, and it covered four of the eight
...
\hsprovenance{Tests: CI run 36108189405 on fd4d929, results.xml, 8 testcases and 0 failures. Mutations: development journal, 17 September 2026. Workflows: runs 36108189395, 36108189405 and 36108189581, 25 September 2026.}
\newpage

\section{The eight tests}\label{sec:tests}
...
\texttt{test\_load\_beats\_count} & with \texttt{LOAD} and \texttt{ENABLE} both high, the loaded value wins with no added one \\ \hline
\texttt{test\_load\_ignored\_while\_driving} & with \texttt{OE} high, \texttt{LOAD} leaves the count alone, and with \texttt{ENABLE} also high it keeps counting \\
\hstoprule
\end{tabularx}
\end{hsblock}

\hsprovenance{Test names and checks from test/test.py at commit fd4d929; all eight passed in CI run 36108189405.}
\newpage
